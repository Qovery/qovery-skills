...
| **qovery-assess** | Read-only assessment of a whole organization — inventories every cluster, environment and service, reads deployment/runtime logs and audit events (redacted on write), then runs 135 best-practice checks across reliability, security, performance, delivery, cost and disaster recovery. Correlates findings to the company's public compliance claims (SOC 2 / ISO 27001 / GDPR / DORA badges), scores each pillar, and writes a customer-shareable gap analysis with a prioritized roadmap. Modifies nothing. Includes `/qovery-assess` slash command |
...
5. **Runs 135 checks with stable IDs** across twelve families — `CP` compliance, `CL` cluster, `TP` topology, `RL` reliability, `BP` anti-patterns, `SC` security, `VS` variables & secrets, `DL` delivery, `LG` logs, `OP` change origin, `CE` cost, `DR` disaster recovery
...
| **5. Security** | `SC-01..SC-21` — database exposure, Kubernetes API CIDRs, unintended public ports, HTTPS, read-only root filesystem, service-account tokens and IRSA, IMDSv2, encryption, SSO, admin minimisation, token scoping, certificates |
