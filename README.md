...
| **qovery-assess** | Read-only assessment of a whole organization — inventories every cluster, environment and service, reads deployment/runtime logs and audit events (redacted on write), then runs 136 best-practice checks across reliability, security, performance, delivery, cost and disaster recovery. Correlates findings to the company's public compliance claims (SOC 2 / ISO 27001 / GDPR / DORA) and maps technical checks to published controls (CIS Kubernetes Benchmark, Pod Security Standards, NSA/CISA, NIST SP 800-190), scores each pillar, and writes a customer-shareable gap analysis with a prioritized roadmap. Modifies nothing. Includes `/qovery-assess` slash command |
...
5. **Runs 136 checks with stable IDs** across twelve families — `CP` compliance, `CL` cluster, `TP` topology, `RL` reliability, `BP` anti-patterns, `SC` security, `VS` variables & secrets, `DL` delivery, `LG` logs, `OP` change origin, `CE` cost, `DR` disaster recovery
...
| **2. Cluster** | `CL-01..CL-16` — health, credentials, K8s version (via `next_k8s_available_version`), production flag, prod/dev isolation, node floor and headroom, observability, KEDA, retention, ingress HA, settings drift |
...
| **4. Reliability** | `RL-01..RL-22` — replica floors, autoscaling headroom, readiness/liveness assessed separately, startup delays, anti-affinity, zone spread, rollout strategy, connection draining, managed databases, backups, stateful SPOFs, pipeline failure rate |
...
| **5. Security** | `SC-01..SC-22` — database exposure, Kubernetes API CIDRs, unintended public ports, HTTPS, read-only root filesystem, service-account tokens and IRSA, IMDSv2, encryption, SSO, admin minimisation, token scoping, certificates |
...
| **6. Delivery** | `DL-01..DL-12` — stage ordering, auto-deploy per tier, monorepo restrictions, webhooks, alert receivers and rules, deploy duration, IaC coverage, mutable image tags |
...
| **6c. Change origin** | `OP-01..OP-06` — Terraform vs Console vs API for production changes, attributable Console edits, interactive shell/port-forward access to production, access-change audit trail, external resources not declared as Qovery Terraform services, failed operations and policy denials |
